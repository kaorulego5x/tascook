  ...
  return (
    <div className={select ? "selected" : "task"} onClick={handleChange}>
      <div className="left">
        <div
          className={
            Math.floor(completedTask / taskSum)
              ? "circle-border-done"
              : "circle-border"
          }
        >
          <div
            className={
              Math.floor(completedTask / taskSum) ? "circle-done" : "circle"
            }
          ></div>
        </div>
        <div
          className={
            Math.floor(completedTask / taskSum) ? "taskName-done" : "taskName"
          }
        >
          {taskName}
        </div>
      ...
      <div className="right">
        <div
          className={
            Math.floor(completedTask / taskSum) ? "calorie-done" : "calorie"
          }
        >
          {calorie}
        </div>
        <div className="task-wrapper">
          ...
          <div className="percentage">
            {Math.floor((completedTask / taskSum) * 100)}%
          </div>
        </div>
